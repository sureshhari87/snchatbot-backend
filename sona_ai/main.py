...
async def recommendations(
    ...
    seed = next(
        (item for item in request.products if str(item.id) == str(request.product_id)), None
    )
    ...
    products, reasons = recommend(
        request.products, seed, request.occasion, request.budget, request.limit
    )
    return RecommendationResponse(
        products=products, reasons=reasons, strategy="content-and-occasion"
    )
...
async def personalized(
    ...
    products, reasons = personalize(
        request.products,
        request.events,
        request.preferred_categories,
        request.preferred_metals,
        request.budget,
        request.limit,
    )
    return RecommendationResponse(
        products=products, reasons=reasons, strategy="privacy-preserving-session-affinity"
    )
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    reply = (
        "I found a few jewellery options that match your request."
        if products
        else "Tell me your preferred jewellery type, metal, occasion, and budget, and I’ll narrow it down."
    )
    source = "rules"
    try:
    ...
    return ChatResponse(
        ...
        confidence=0.82 if products else 0.55,
        answer_source=source,
        tool_calls=["catalog_search"],
        guardrails=["catalog_grounded", "verify_final_price_and_size"],
    )
...
async def concepts(
    ...
    generated = generated or {
        ...
        "craft_notes": [
            "Confirm dimensions, weight, stone setting, hallmarking, and final estimate with a jeweller."
        ],
    }
    image = None
